...
namespace AODL.Document.Import.OpenDocument.NodeProcessors {
	...
	internal class MainContentProcessor {
		...
		private TextList CreateList (XmlNode listNode) {
			try {
				//Create a new List
				TextList list                   = new TextList(this._document, listNode);
				IContentCollection iColl    = new IContentCollection();
		...
		/// <param name="node">The node.</param>
		/// <returns></returns>
		private TextListItem CreateListItem (XmlNode node) {
			try {

				TextListItem listItem           = new TextListItem(this._document);
				IContentCollection iColl    = new IContentCollection();
				listItem.Node = node;
