...
namespace AODL.Document.Content.Text.Indexes {
	...
	public class TableOfContentsSource {
		...
		private void NewXmlNode () {
			...
			//be modifiable.
			XmlAttribute xa                 = this.TableOfContents.Document.CreateAttribute("outline-level", "text");
			xa.Value = "10";
			...
			XmlNode indexTitleTemplateNode  = this.TableOfContents.Document.CreateNode("index-title-template", "text");

			indexTitleTemplateNode.InnerText = this.TableOfContents.Title;

			//Fixed style for the title template
			xa = this.TableOfContents.Document.CreateAttribute ("style-name", "text");
