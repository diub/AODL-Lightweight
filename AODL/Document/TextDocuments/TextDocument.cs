		...
		*/
		...
		public void AddFont (string fontname) {
			try {
				...
				// diub - Dipl.-Ing. Uwe Barth 2021-04-06 - 13.08.55
				// Wie kann man das hier weglassen ?? Unbegreiflich!
				else {
					Font real_new_font;
					FontFamily real_font_family;
					string pitch;
					float diff;

					real_new_font = new System.Drawing.Font (fontname, 11);
					real_font_family = real_new_font.FontFamily;
					pitch = "variable";
					if (real_font_family.IsStyleAvailable (FontStyle.Regular)) {
						diff = TextRenderer.MeasureText ("WWW", real_new_font).Width - TextRenderer.MeasureText ("...", real_new_font).Width;
					...
					foreach (XmlNode xn in fontsnode)
						if (xn.Name == "office:font-face-decls") {
							...
							xanew = this.CreateAttribute ("font-family", "svg");
							xanew.Value = real_font_family.Name;
							node.Attributes.Append (xanew);

							xanew = this.CreateAttribute ("font-pitch", "style");
							xanew.Value = pitch;
							node.Attributes.Append (xanew);
